...
(function initTheme() {
    ...
    btn.addEventListener('click', function () {
        ...
        try { localStorage.setItem('wfcs-docs-theme', next); } catch (e) {}
...
function saveToken(token) {
    try { localStorage.setItem('wfcs_agent_token', token); } catch (e) {}
}

function loadToken() {
    try { return localStorage.getItem('wfcs_agent_token'); } catch (e) { return null; }
}

function clearToken() {
    try { localStorage.removeItem('wfcs_agent_token'); } catch (e) {}
}
...
async function ensureCsrfToken() {
    if (!getCsrfToken()) {
        ...
        try { await fetch(API_BASE + '/', { credentials: 'include' }); } catch (e) {}
    }
}
...
async function handleLogin() {
    ...
    try {
        ...
        var response = await fetch(API_BASE + '/api/auth/login', {
            ...
            credentials: 'include',
            body: JSON.stringify({ email: email, password: password, _csrf_token: csrfToken })
        });
...
function renderPrompts() {
    ...
    Object.keys(allPrompts).forEach(function (category) {
        ...
        allPrompts[category].forEach(function (prompt) {
            if (!searchTerm || prompt.text.toLowerCase().includes(searchTerm)) {
                results.push({ text: prompt.text, difficulty: prompt.difficulty, category: category });
            }
        });
    });
    ...
    if (results.length === 0) {
        ...
        empty.textContent = 'No prompts found. Try a different search term or filter.';
...
function difficultyLabel(d) {
    return { easy: 'Easy', intermediate: 'Intermediate', advanced: 'Advanced' }[d] || d;
}
